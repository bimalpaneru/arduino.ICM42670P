 ...
 * (collectively "Software") is subject to InvenSense, Inc and its licencors'
 * intellectual property rights under U.S. and international copyright and other
 * intellectual property rights laws.
 *
 * InvenSense, Inc and its licencors retain all intellectual property and
 ...
 * agreement from InvenSense, Inc is strictly prohibited.
 *
 ...
 * InvenSense, Inc BE LIABLE FOR ANY DIRECT, SPECIAL, INDIRECT, INCIDENTAL, OR
...
static int i2c_write(struct inv_imu_serif *serif, uint8_t reg,
                     const uint8_t *wbuffer, uint32_t wlen);
static int i2c_read(struct inv_imu_serif *serif, uint8_t reg, uint8_t *rbuffer,
                    uint32_t rlen);
static int spi_write(struct inv_imu_serif *serif, uint8_t reg,
                     const uint8_t *wbuffer, uint32_t wlen);
static int spi_read(struct inv_imu_serif *serif, uint8_t reg, uint8_t *rbuffer,
                    uint32_t rlen);
static void event_cb(inv_imu_sensor_event_t *event);

// i2c and SPI interfaces are used from C driver callbacks, without any
// knowledge of the object As they are declared as static, they will be
// overriden each time a new ICM42670P object is created i2c
...
// This is used by the getDataFromFifo callback (not object aware), declared
// static
...
int ICM42670P::begin() {
  ...
  icm_serif.max_read =
      2048; /* maximum number of bytes allowed per serial read */
  icm_serif.max_write =
      2048; /* maximum number of bytes allowed per serial write */
  ...
  if ((who_am_i != ICM42670P_WHOAMI) && (who_am_i != ICM42670S_WHOAMI) &&
      (who_am_i != ICM42670T_WHOAMI)) {
    return -3;
  }
...
int ICM42670P::enableFifoInterrupt(uint8_t intpin,
                                   ICM42670P_irq_handler handler,
  ...
  // Set fifo_wm_int_w generating condition : fifo_wm_int_w generated when
  // counter == threshold
