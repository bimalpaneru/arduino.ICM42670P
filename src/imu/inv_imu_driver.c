...
void inv_imu_sleep_us(uint32_t us) { delayMicroseconds(us); }

uint64_t inv_imu_get_time_us(void) { return (uint64_t)micros(); }
...
int inv_imu_get_who_am_i(struct inv_imu_device *s, uint8_t *who_am_i) { return inv_imu_read_reg(s, WHO_AM_I, 1, who_am_i); }
...
int inv_imu_enable_accel_low_power_mode(struct inv_imu_device *s) {
    ...
    if ((accel_mode != PWR_MGMT0_ACCEL_MODE_LP) && ((gyro_mode == PWR_MGMT0_GYRO_MODE_OFF) || (gyro_mode == PWR_MGMT0_GYRO_MODE_STANDBY))) {
    ...
    if (accel_mode == PWR_MGMT0_ACCEL_MODE_OFF) {
        ...
        */
        if (s->fifo_is_used) s->accel_start_time_us = inv_imu_get_time_us();
    }
...
int inv_imu_disable_accel(struct inv_imu_device *s) {
    ...
    if ((gyro_mode == PWR_MGMT0_GYRO_MODE_OFF) && ((fifo_cfg_6_reg & FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_MASK) == 0)) {
        /*
         * Disable the automatic RCOSC power on to avoid extra power consumption
         * in sleep mode (all sensors and clocks off)
         */
        fifo_cfg_6_reg |= ((1 & FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_MASK) << FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_POS);
...
int inv_imu_enable_gyro_low_noise_mode(struct inv_imu_device *s) {
    ...
    if (s->gyro_power_off_tmst != UINT32_MAX) {
        ...
        /* Handle rollover */
        if (current_time <= s->gyro_power_off_tmst) current_time += UINT32_MAX;
        /* If 20 ms are not elapsed since power-off error is returned */
        if ((current_time - s->gyro_power_off_tmst) <= GYR_POWER_OFF_DUR_US) return INV_ERROR_HW;
    }
    ...
    if (gyro_mode == PWR_MGMT0_GYRO_MODE_OFF) {
        ...
        */
        if (s->fifo_is_used) s->gyro_start_time_us = inv_imu_get_time_us();
    }
...
int inv_imu_disable_gyro(struct inv_imu_device *s) {
    ...
    status |= inv_imu_read_reg(s, FIFO_CONFIG6_MREG1, 1, &fifo_cfg_6_reg);
    if ((accel_mode == PWR_MGMT0_ACCEL_MODE_OFF) && ((fifo_cfg_6_reg & FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_MASK) == 0)) {
...
int inv_imu_set_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it) {
    ...
    data[0] &= ~(INT_SOURCE0_FSYNC_INT1_EN_MASK | INT_SOURCE0_DRDY_INT1_EN_MASK | INT_SOURCE0_FIFO_THS_INT1_EN_MASK | INT_SOURCE0_FIFO_FULL_INT1_EN_MASK);
    ...
    data[1] &= ~(INT_SOURCE1_SMD_INT1_EN_MASK | INT_SOURCE1_WOM_X_INT1_EN_MASK | INT_SOURCE1_WOM_Y_INT1_EN_MASK | INT_SOURCE1_WOM_Z_INT1_EN_MASK);
    ...
    data[0] &= ~(INT_SOURCE6_FF_INT1_EN_MASK | INT_SOURCE6_LOWG_INT1_EN_MASK | INT_SOURCE6_STEP_DET_INT1_EN_MASK | INT_SOURCE6_STEP_CNT_OFL_INT1_EN_MASK | INT_SOURCE6_TILT_DET_INT1_EN_MASK);
...
int inv_imu_set_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it) {
    ...
    data[0] &= ~(INT_SOURCE3_FSYNC_INT2_EN_MASK | INT_SOURCE3_DRDY_INT2_EN_MASK | INT_SOURCE3_FIFO_THS_INT2_EN_MASK | INT_SOURCE3_FIFO_FULL_INT2_EN_MASK);
    ...
    data[1] &= ~(INT_SOURCE4_SMD_INT2_EN_MASK | INT_SOURCE4_WOM_X_INT2_EN_MASK | INT_SOURCE4_WOM_Y_INT2_EN_MASK | INT_SOURCE4_WOM_Z_INT2_EN_MASK);
    ...
    data[0] &= ~(INT_SOURCE7_FF_INT2_EN_MASK | INT_SOURCE7_LOWG_INT2_EN_MASK | INT_SOURCE7_STEP_DET_INT2_EN_MASK | INT_SOURCE7_STEP_CNT_OFL_INT2_EN_MASK | INT_SOURCE7_TILT_DET_INT2_EN_MASK);
...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s) {
    ...
    if ((int_status & INT_STATUS_FIFO_THS_INT_MASK) || (int_status & INT_STATUS_FIFO_FULL_INT_MASK)) {
        ...
        while (packet_count > 0) {
            ...
            if (s->fifo_highres_enabled) packet_size = FIFO_20BYTES_PACKET_SIZE;
                ...
                if (header->Byte == 0x80) {
                    ...
                    if ((header->bits.accel_bit) || (header->bits.gyro_bit)) {
                        ...
                        if (header->bits.twentybits_bit) {
                            ...
                            if (event.temperature != INVALID_VALUE_FIFO) event.sensor_mask |= (1 << INV_SENSOR_TEMPERATURE);
                        } else {
                            ...
                            /* new temperature data */
                            if (event.temperature != INVALID_VALUE_FIFO_1B) event.sensor_mask |= (1 << INV_SENSOR_TEMPERATURE);
                        }
                    }
                    ...
                    if (header->bits.gyro_bit) {
                        if ((event.gyro[0] != INVALID_VALUE_FIFO) && (event.gyro[1] != INVALID_VALUE_FIFO) && (event.gyro[2] != INVALID_VALUE_FIFO)) {
                    ...
                    if (header->bits.twentybits_bit) fifo_idx += FIFO_ACCEL_GYRO_HIGH_RES_SIZE;
...
int inv_imu_reset_fifo(struct inv_imu_device *s) {
    ...
    while ((status == 0) && ((fifo_flush_status & SIGNAL_PATH_RESET_FIFO_FLUSH_MASK) == (uint8_t)SIGNAL_PATH_RESET_FIFO_FLUSH_EN)) {
    ...
    switch (fifo_config) {
        case INV_IMU_FIFO_ENABLED:
            ...
            */
            ...
            data &= ~(FIFO_CONFIG5_FIFO_GYRO_EN_MASK | FIFO_CONFIG5_FIFO_ACCEL_EN_MASK | FIFO_CONFIG5_FIFO_TMST_FSYNC_EN_MASK | FIFO_CONFIG5_FIFO_HIRES_EN_MASK);
            data |= ((uint8_t)FIFO_CONFIG5_GYRO_EN | (uint8_t)FIFO_CONFIG5_ACCEL_EN | (uint8_t)FIFO_CONFIG5_TMST_FSYNC_EN | (uint8_t)FIFO_CONFIG5_WM_GT_TH_EN);
        ...
        case INV_IMU_FIFO_DISABLED:
            ...
            /* restart and reset FIFO configuration */
            status |= inv_imu_read_reg(s, FIFO_CONFIG5_MREG1, 1, &data);
            data &= ~(FIFO_CONFIG5_FIFO_GYRO_EN_MASK | FIFO_CONFIG5_FIFO_ACCEL_EN_MASK | FIFO_CONFIG5_FIFO_TMST_FSYNC_EN_MASK);
            data |= ((uint8_t)FIFO_CONFIG5_GYRO_DIS | (uint8_t)FIFO_CONFIG5_ACCEL_DIS | (uint8_t)FIFO_CONFIG5_TMST_FSYNC_EN);
            status |= inv_imu_write_reg(s, FIFO_CONFIG5_MREG1, 1, &data);
